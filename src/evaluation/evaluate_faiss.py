...
def process_results(gen: Generator, default_rank: int = None, csv_path: str = None, all_posts: bool = False):
    ...
    for predicted_ids, desired_ids, post_id in gen:
        ...
        if csv_path:
            rows.append((post_id, post_ranks, predicted_ids))
...
def evaluate_multiclaim_retrieval(dataset, pipeline, prompt_template, csv_path=None):
    post_ids = list(dataset.id_to_post.keys())
    posts = list(dataset.id_to_post.values())
    
    # posts = [prompt_template.replace('<<input>>', post) for post in posts]
    # print(posts[1])
    ...
    df = pd.DataFrame(columns=['post_id', 'fact_check_ids', 'prompt1', 'prompt2'])
    
    output = pipeline(query=query)
    fact_check_ids = output.get('top_k', np.array([]))
    
    print(len(post_ids), len(fact_check_ids))

    if csv_path is not None:
        results = {
            'post_id': post_ids,
            'fact_check_ids': fact_check_ids,
            'prompt1': [prompt_template] * len(post_ids),
            'prompt2': posts
        }
        
        # print(len(fact_check_ids), len(sims))
        # results = {
        #     'post_id': post_ids[0],
